...
var Job = require('./job.js'), humanInterval = require('human-interval'), utils = require('util'),
  Emitter = require('events').EventEmitter, debug = require('debug')('agenda:worker');
...
function processJobs(extraJob) {
  ...
  if (!extraJob) {
    ...
    debug('job was passed directly to processJobs(), locking and running immediately');
  ...
  function lockOnTheFly() {
    ...
    self._collection.findOneAndUpdate(criteria, update, options, function(err, resp) {
      ...
      if (resp.value) {
        debug('found a job that can be locked on the fly in MongoDB');
  ...
  function processJobResult(err, job) {
    ...
    if (self._runningJobs.indexOf(job) === -1) {
      ...
      throw ("callback already called - job " + name + " already marked complete");
    }
